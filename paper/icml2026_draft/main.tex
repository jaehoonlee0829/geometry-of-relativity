...
We next test whether the behavioral $z$ readout depends on incidental details of the prompt generator: Gaussian-shaped contexts, random example order, or ordinary in-distribution value ranges.
Across non-Gaussian context shapes (\cref{fig:distribution-shapes}) and ordering changes, the LD-by-$z$ relationship remains positive and mostly monotone; full curves are shown in \cref{fig:distribution-robustness-full,fig:order-robustness-full}.

\begin{figure}[!tbp]
  \centering
  \includegraphics[width=\linewidth,keepaspectratio]{figures/fig_results_distribution_shapes_clean.png}
  \caption{\textbf{Distribution-shape controls.}
  Context examples were sampled from normal, uniform, U-shaped, skewed, and bimodal distributions, then standardized to the same mean and variance.
  This isolates whether the model requires the original Gaussian context shape; full LD-by-$z$ curves are shown in \cref{fig:distribution-robustness-full}.
  % Source artifacts: figures/v14/distribution/distribution_shape_examples.png and results/v14/distribution/distribution_rows.jsonl, replotted for paper display.
  }
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth,keepaspectratio]{figures/fig_results_ood_ld_by_z_main.png}
  \caption{\textbf{OOD worlds preserve the relative-standing readout.}
  High-minus-low adjective logit difference remains ordered by $z$ in ordinary in-distribution contexts and artificial low/high OOD worlds.
  Full eight-domain curves: \cref{fig:ood-robustness-full}.
  % Source artifact: results/v14_1/affine_ood/affine_ood_rows.jsonl, corrected rerun with complete pair/condition coverage.
  }
  \label{fig:ood-robustness}
\end{figure}
...
The sharpest stress test is the OOD-world setting, where raw magnitude and local standing can point in opposite semantic directions:
\begin{itemize}
...
\end{itemize}
These are individual corrected-run cells averaged over repeated context seeds at $N=31$.
