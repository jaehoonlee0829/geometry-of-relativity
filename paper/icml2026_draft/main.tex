...
The first control changes only the order of the same context multiset.
...
The second control changes the shape of the context distribution while standardizing the requested mean and variance.
Normal and bimodal contexts are the cleanest contrast: in the bimodal case, middle values lie between two clusters rather than inside a single dense central mass, so their local neighborhood is less representative of the global mean and variance.
This appears as small mid-$z$ deviations, but the LD-by-$z$ curves remain ordered by relative standing rather than collapsing to raw magnitude.
