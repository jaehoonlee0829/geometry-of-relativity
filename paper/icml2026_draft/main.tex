...
\begin{figure}[t]
  ...
  Direct steering has a substantially larger effect along \primalz{} than along \primalx{} when applied across the same layer sweep (bottom).
...
\begin{figure}[t]
  ...
  For Gemma 2 9B at layer 33, the plot compares steering with each concept's own \primalz{} direction against steering with a single sign-aligned mean of the eight per-concept \primalz{} directions.
...
We then form a shared direction by sign-aligning these eight concept directions to their initial mean and averaging them.
\Cref{fig:shared-direction-steering} shows that this shared direction recovers at least half of the within-concept steering effect for most concepts, suggesting that relative standing is partly represented in a common residual-stream direction rather than only in concept-specific axes.
\Cref{fig:z-vs-x-transfer} provides a complementary cross-pair control: transfer is substantially stronger for \primalz{} than for raw-$x$ directions.
...
\begin{figure*}[!tbp]
  ...
  In Gemma 2 9B at layer 33, rows are target concepts and columns are source directions.
