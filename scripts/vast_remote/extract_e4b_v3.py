"""v3 extractor: save W_U once per model; per-layer npz is activations-only (tiny, fast)."""
...
SHORT = "e4b"
LAYER_INDICES = {"early": 10, "mid": 21, "late": 32, "final": 41}
...
def extract_one_domain(model, tok, prompts, ids, domain, layer_indices, batch_size=8):
...
def main():
    ...
    # Save W_U ONCE, uncompressed (np.save is ~50x faster than savez_compressed on big arrays)
    ...
    for domain, ts in by_dom.items():
        ...
        t0 = time.time()
        per_layer_acts = extract_one_domain(model, tok, prompts, ids, domain, LAYER_INDICES, batch_size=8)
        dt = time.time() - t0
        ...
        for layer_key, acts in per_layer_acts.items():
            ...
            t1 = time.time()
            np.savez(  # uncompressed — activations are tiny (252*2560*4 = 2.5MB)
                out_path,
